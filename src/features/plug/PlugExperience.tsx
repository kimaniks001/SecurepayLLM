...
import { ArrowLeft, HandHelping, ArrowRight, ShieldCheck, AlertTriangle } from 'lucide-react';
  ...
  if (agreementId && state.existingAttribution.status === 'ready') {
    ...
    body = (
      <div className="max-w-2xl mx-auto px-4 md:px-6 py-4 space-y-4">
        <div className="rounded-2xl border border-forest-200 bg-forest-50/20 px-5 py-4">
        ...
        {referral && (
          <div className="rounded-2xl border border-cream-200 bg-white px-5 py-4">
            ...
            {referral.reward && (
              <p className="text-[0.8rem] text-sand-600 mt-1">Referral reward: {referral.reward.amountMinor} {referral.reward.currency} (minor units)</p>
            )}
    ...
    body = (
      <div className="max-w-2xl mx-auto px-4 md:px-6 py-4 space-y-4">
        ...
        {agreementId && (
          <>
            {state.attributionError && <p role="alert" className="text-[0.78rem] text-ember-600 px-1">{state.attributionError}</p>}
            <button
    ...
    body = (
      <div className="max-w-2xl mx-auto px-4 md:px-6 py-4 space-y-4">
        ...
        {requestId && (
          <button onClick={() => void controller.loadCandidates(requestId)} className="w-full flex items-center justify-center gap-2 rounded-xl bg-forest-600 text-cream-50 text-[0.875rem] font-medium py-3">
        ...
        {agreementId && state.existingAttribution.status === 'error' && (
          <div className="rounded-lg bg-ember-50 border border-ember-200 px-3 py-2 flex items-start gap-2">
            <AlertTriangle className="w-3.5 h-3.5 text-ember-600 mt-0.5 shrink-0" />
            <p className="text-[0.72rem] text-ember-700">{errorText(state.existingAttribution.error)}</p>
          </div>
        )}
      </div>
    );
