...
import { ArrowLeft, ShieldCheck, Award, Check, ArrowRight, FileText, AlertCircle } from 'lucide-react';
...
/**
 * Real, non-dispute Master domain (task section 21 — never the separate DisputeMasterEscalation flow).
 * There is no Master directory/search endpoint (task section 7, confirmed absent from source) — a person
 * reaches a profile only by an identityId reference they already have (e.g. from an Agent conversation or
 * a prior opinion); this screen renders that gap honestly rather than fabricating a browsable list.
 * `MasterProfileCard`/`MasterRequestView`/`MasterOpinionView` stay fixture-only: their prop shapes assume a
 * display name and a pre-existing fixed cost/appointment status the real state machine does not have (see
 * docs/PRODUCTION_MIGRATION_LEDGER.md section 18), so this is a new, narrow real screen in the same visual
 * language — the same divergence Golden Spine G used for Circle's named-Circle gap.
 */
export function MasterExperience({ gateway, auth, session, initialIdentityId, onNavigate }: {
  ...
  useEffect(() => {
    ...
    // Runs once on mount only — initialIdentityId is stable for this component's lifetime (see AgentExperience keying).
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // A previous identity's Master request/opinion draft must never survive into a signed-out or newly
  // re-authenticated session (task section 19); the looked-up profile itself is a public read and is left
  // alone. Any in-progress request/opinion screen falls back to the (still-valid, public) profile view.
  useEffect(() => {
  ...
  // Replays the exact action that was gated on sign-in, once, after a real session actually exists —
  // never inferred from sign-in alone (task section 10: "no Agent, Plug or staff member... on somebody
  // else's behalf" — this only ever replays the same caller's own explicit prior click).
  useEffect(() => {
      ...
      <div className="max-w-2xl mx-auto px-4 md:px-6 py-4 space-y-4">
        <div className="rounded-2xl border border-cream-200 bg-white px-5 py-4">
          ...
          <p className="text-[0.825rem] text-sand-600 mb-3">
            SecurePay does not yet have a searchable Master directory. If you already have a specific
            Master's reference (from a conversation with SecurePay or a prior opinion), enter it below.
          </p>
          <input
            value={lookupInput}
            onChange={e => setLookupInput(e.target.value)}
            placeholder="Master reference (identity id)"
            className="w-full rounded-lg border border-cream-200 px-3 py-2 text-[0.85rem] mb-3"
          />
          <button
            onClick={() => { void controller.lookupProfile(lookupInput); setView('profile'); }}
            disabled={!lookupInput.trim()}
            className="w-full flex items-center justify-center gap-2 rounded-xl bg-forest-600 text-cream-50 text-[0.875rem] font-medium py-3 hover:bg-forest-700 transition-colors disabled:opacity-40"
          >
            Look up Master
        ...
        <div className="max-w-2xl mx-auto px-4 md:px-6 py-4 space-y-4">
          <div className="rounded-2xl border border-cream-200 bg-white px-5 py-4 animate-quiet-in">
            ...
            <div className="mt-3">
              ...
              <div className="flex flex-wrap gap-1.5">
                {master.expertiseDomains.map((exp, i) => <span key={i} className="text-[0.72rem] text-forest-700 bg-forest-50 border border-forest-100 rounded-full px-2 py-0.5">{exp}</span>)}
              </div>
            </div>
            ...
            {master.accreditationRefs.length > 0 && (
              <div className="mt-2">
                <div className="text-[0.72rem] font-medium text-sand-500 mb-1">Accreditation references</div>
                <ul className="space-y-0.5">{master.accreditationRefs.map((a, i) => <li key={i} className="text-[0.78rem] text-forest-800">· {a}</li>)}</ul>
              </div>
            )}
          </div>
          <div className="rounded-2xl border border-cream-200 bg-cream-50/50 px-5 py-4">
            ...
            <p className="text-[0.78rem] text-sand-600">
              A qualification/accreditation reference is shown exactly as SecurePay recorded it. Master
              designation is not itself a statutory licence, certification, or SecurePay endorsement.
            </p>
          </div>
          <button
            onClick={() => requireAuth(() => { controller.setDraft({ masterIdentityId: master.identityId }); setView('request-create'); })}
    ...
    body = (
      <div className="max-w-2xl mx-auto px-4 md:px-6 py-4 space-y-4">
        <div className="rounded-2xl border border-cream-200 bg-white px-5 py-4 animate-quiet-in space-y-3">
          ...
          <p className="text-[0.72rem] text-sand-500">
            This is what will be sent. Review it before submitting — nothing is requested until you submit.
          </p>
          <div>
            <label className="text-[0.72rem] text-sand-500">What kind of engagement is this?</label>
            <select value={state.draft.sourceContext} onChange={e => controller.setDraft({ sourceContext: e.target.value as MasterRequestSourceContext })} className="w-full rounded-lg border border-cream-200 px-3 py-2 text-[0.85rem] mt-1">
              {SOURCE_CONTEXTS.map(c => <option key={c.value} value={c.value}>{c.label}</option>)}
            </select>
          </div>
          <div>
            <label className="text-[0.72rem] text-sand-500">Related Agreement (optional)</label>
            <input value={state.draft.agreementId} onChange={e => controller.setDraft({ agreementId: e.target.value })} placeholder="Agreement id, if this relates to one" className="w-full rounded-lg border border-cream-200 px-3 py-2 text-[0.85rem] mt-1" />
          </div>
          <div>
            <label className="text-[0.72rem] text-sand-500">Question</label>
            <textarea value={state.draft.question} onChange={e => controller.setDraft({ question: e.target.value })} rows={2} className="w-full rounded-lg border border-cream-200 px-3 py-2 text-[0.85rem] mt-1" />
          </div>
          <div>
            <label className="text-[0.72rem] text-sand-500">Scope</label>
            <textarea value={state.draft.scope} onChange={e => controller.setDraft({ scope: e.target.value })} rows={2} className="w-full rounded-lg border border-cream-200 px-3 py-2 text-[0.85rem] mt-1" />
          </div>
          <div>
            <label className="text-[0.72rem] text-sand-500">Evidence references (one per line, optional)</label>
            <textarea value={state.draft.evidenceRefs} onChange={e => controller.setDraft({ evidenceRefs: e.target.value })} rows={2} className="w-full rounded-lg border border-cream-200 px-3 py-2 text-[0.85rem] mt-1" />
          </div>
          <label className="flex items-center gap-2 text-[0.8rem] text-sand-600">
            <input type="checkbox" checked={state.draft.siteVisitRequired} onChange={e => controller.setDraft({ siteVisitRequired: e.target.checked })} />
            Site visit required
          </label>
          {state.request.status === 'error' && <p role="alert" className="text-[0.78rem] text-ember-600">{errorText(state.request.error, true)}</p>}
          <button
            onClick={() => void controller.submitRequest().then(() => setView('request-detail'))}
      ...
      body = (
        <div className="max-w-2xl mx-auto px-4 md:px-6 py-4 space-y-4">
          ...
          <div className="rounded-2xl border border-cream-200 bg-white px-5 py-4">
            ...
            {req.currency && req.quotedCostMinor && (
              <p className="text-[0.82rem] text-sand-600 mt-1">Quoted cost: {req.quotedCostMinor} {req.currency} (minor units)</p>
            )}
          </div>
          {state.requestActionError && <p role="alert" className="text-[0.78rem] text-ember-600 px-1">{state.requestActionError}</p>}
          ...
          {req.status === 'REQUESTED' && (
            <div className="rounded-2xl border border-cream-200 bg-white px-5 py-4 space-y-2">
              ...
              <div className="flex gap-2">
                <input value={costCurrency} onChange={e => setCostCurrency(e.target.value)} className="w-20 rounded-lg border border-cream-200 px-2 py-2 text-[0.85rem]" />
                <input value={costAmount} onChange={e => setCostAmount(e.target.value.replace(/[^0-9]/g, ''))} placeholder="Amount (minor units)" className="flex-1 rounded-lg border border-cream-200 px-3 py-2 text-[0.85rem]" />
              </div>
              <button
                onClick={() => requireAuth(() => void controller.proposeCost(req.id, costCurrency.trim(), Number(costAmount || 0)))}
                disabled={!costCurrency.trim() || !costAmount || state.requestActionBusy}
                className="w-full rounded-xl bg-forest-600 text-cream-50 text-[0.825rem] font-medium py-2.5 disabled:opacity-40"
              >
                Propose cost
              </button>
              <button onClick={() => requireAuth(() => void controller.declineRequest(req.id))} disabled={state.requestActionBusy} className="w-full rounded-xl border border-cream-200 text-sand-600 text-[0.825rem] font-medium py-2.5 disabled:opacity-40">Decline (Master only)</button>
            </div>
          )}
          {req.status === 'COST_PROPOSED' && (
            <div className="rounded-2xl border border-cream-200 bg-white px-5 py-4 space-y-2">
              <div className="rounded-lg bg-ember-50 border border-ember-200 px-3 py-2 flex items-start gap-2">
                <AlertCircle className="w-3.5 h-3.5 text-ember-600 mt-0.5 shrink-0" />
                <p className="text-[0.72rem] text-ember-700">Accepting this cost does not create or move Money. Payment for Master services is a separate path, not automatic from Agreement funds.</p>
              </div>
              <button onClick={() => requireAuth(() => void controller.acceptCost(req.id))} disabled={state.requestActionBusy} className="w-full rounded-xl bg-forest-600 text-cream-50 text-[0.825rem] font-medium py-2.5 disabled:opacity-40">Accept cost (Requester only)</button>
              <button onClick={() => requireAuth(() => void controller.declineRequest(req.id))} disabled={state.requestActionBusy} className="w-full rounded-xl border border-cream-200 text-sand-600 text-[0.825rem] font-medium py-2.5 disabled:opacity-40">Decline (Master only)</button>
            </div>
          )}
          {req.status === 'ACCEPTED' && (
            <button onClick={() => requireAuth(() => setView('opinion-form'))} className="w-full rounded-xl bg-forest-600 text-cream-50 text-[0.825rem] font-medium py-2.5">Submit opinion (Master only)</button>
          )}
          {(req.status === 'DECLINED' || req.status === 'CANCELLED') && (
            <p className="text-[0.78rem] text-sand-500 px-1">This request is closed and cannot progress further.</p>
          )}
          {req.status === 'OPINION_SUBMITTED' && (
            <p className="text-[0.78rem] text-forest-600 px-1">An opinion has been submitted for this request.</p>
          )}
          <p className="text-[0.68rem] text-sand-400 italic px-2">
            Master opinion does not automatically change Agreement, resolve dispute, or release Money.
          </p>
        </div>
      );
    }
    ...
    body = (
      <div className="max-w-2xl mx-auto px-4 md:px-6 py-4 space-y-4">
        <div className="rounded-2xl border border-cream-200 bg-white px-5 py-4 space-y-3">
          ...
          {state.requestActionError && <p role="alert" className="text-[0.78rem] text-ember-600">{state.requestActionError}</p>}
          <button
            onClick={() => { if (req) void controller.submitOpinion(req.id).then(() => setView('request-detail')); }}
            disabled={!state.opinionDraft.opinionText.trim() || state.requestActionBusy || !req}
  ...
  return (
    <div className="min-h-dvh flex flex-col bg-cream-100 pb-16 md:pb-0">
      ...
      <div className="px-4 md:px-6 py-3 border-b border-cream-200/60 bg-cream-50">
        <button onClick={() => (view === 'lookup' ? onNavigate('ecosystem') : setView(view === 'profile' ? 'lookup' : 'profile'))} className="flex items-center gap-1.5 text-[0.8rem] text-sand-500 hover:text-forest-600 transition-colors">
