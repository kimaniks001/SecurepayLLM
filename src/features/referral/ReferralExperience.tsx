...
/** Phase 4 correction: the reward amount was previously rendered as raw minor units (e.g. "50000
 * KES (minor units)") -- never divided by 100 or formatted the same way every other money display in
 * this codebase already is. This is a presentational fix only; rewardPaid/eligibility remain entirely
 * backend-owned. */
        ...
        <Surface>
          <SurfaceBody>
            ...
            <div className="flex gap-2">
              ...
              <Button onClick={() => void controller.redeem()} disabled={!state.redeemInput.trim() || state.redeemBusy} className="px-4">Redeem</Button>
            </div>
            {state.redeemError && <StatusNotice tone="warning" icon={false} className="mt-2">{state.redeemError}</StatusNotice>}
            {state.redeemedStatus && <StatusNotice tone="success" icon={false} className="mt-2">Redeemed — status: {state.redeemedStatus}</StatusNotice>}
          </SurfaceBody>
        </Surface>

        {history.relationships.map(rel => (
          <Surface key={rel.relationshipId}>
            <SurfaceBody>
              ...
              {rel.reward && (
                <div className="mt-1.5 text-[0.78rem] text-sand-600">Referral reward: <MoneyValue amount={money(rel.reward.amountMinor, rel.reward.currency)} size="sm" /></div>
              )}
