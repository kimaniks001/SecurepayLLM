...
export interface InviteDraft {
  ...
  sent: boolean;
  /**
   * Correction (Slice 2 pre-merge) -- minted ONCE when the invite panel opens (`openInvite`), never
   * regenerated by `submitInvite` itself. It survives local validation failures, network failures,
   * ambiguous/backend rejections, and any number of repeated Submit presses against this same
   * unchanged draft (including after the caller edits the KS Number field while the panel remains
   * open -- one open invitation *intention*, one key, exactly the same lesson as `ComposeDraft`'s own
   * `idempotencyKey`). Discarded only on confirmed success or an explicit cancel/new invitation
   * session (`openInvite` mints a fresh one).
   */
  idempotencyKey: string;
}
const emptyInviteDraft: InviteDraft = { ksNumber: '', submitting: false, error: null, sent: false, idempotencyKey: '' };
...
export function createCommunityController(
  ...
  return {
    ...
    openInvite() { update({ inviteOpen: true, inviteDraft: { ...emptyInviteDraft, idempotencyKey: newIdempotencyKey('community-invite') } }); },
    ...
    async submitInvite() {
      ...
      try {
        // The SAME idempotencyKey minted when the invite panel opened is reused on every attempt
        // against this unchanged draft -- never regenerated here, so a retry after any failure
        // (local validation never reaches this point; network, ambiguous, or backend rejection all
        // do, and so does changing the KS Number while the panel remains open) stays a genuine retry
        // of the same invitation intention rather than risking duplicate invite semantics.
        await community.membership.invite(ksNumber, state.inviteDraft.idempotencyKey);
        // Confirmed success ends this invitation intention -- a fresh key is minted immediately so
        // that inviting someone else without closing the panel starts its own new intention.
