...
import { cctvComparison } from '../storeData';
...
export function OfferComparisonView() {
  const data = cctvComparison;
  return (
    <div className="rounded-2xl border border-cream-200 bg-white shadow-card overflow-hidden animate-quiet-in">
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-cream-200">
              ...
              {data.offers.map((o, i) => (
                <th key={i} className="text-left px-4 py-2.5 min-w-[100px]">
                  <div className="text-[0.825rem] font-medium text-forest-800">{o.name}</div>
                  <div className="text-[0.7rem] text-sand-500">{o.store}</div>
                </th>
              ))}
            </tr>
          </thead>
          <tbody>
            {data.rows.map((row, i) => (
              <tr key={i} className={`border-b border-cream-100 last:border-0 ${i % 2 === 0 ? 'bg-cream-50/50' : ''}`}>
