 ...
 */
export function InvitationsForYou({ items, onReview }: InvitationsForYouProps) {
  ...
  return (
    <div className="animate-quiet-in">
      ...
      {remaining > 0 && (
        <p className="mt-2 text-[0.75rem] text-sand-500">+{remaining} more invitation{remaining === 1 ? '' : 's'}</p>
      )}
    </div>
  );
}
