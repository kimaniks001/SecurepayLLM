...
| What records confirmation? | `POST /agreements/{id}/versions/{vid}/confirm`, **auth + joined participant**: participant → `CONFIRMED`, confirmation row with `assuranceMethod=AUTHENTICATED_SESSION`. Triggers Payment Ready evaluation server-side (best effort) | "Yes, this is what I agree to" (action value `confirm_acceptance`) | wording; now `confirm_version` |
...
- **Confirm CTA:** "Yes, I confirm this version" (was "Yes, this is what I agree to"); the recipient action value is now `confirm_version` (was `confirm_acceptance`, which survives only in fixture/mock code `App.tsx`/`mockAgent.ts`, not the production recipient path): the backend records a participant confirming an exact version via authenticated session; it does not model a signature, so no "sign"/"legally".
...
- **Opening is a read with a side effect:** `GET agreement-invitations/{token}` records `VIEWED` on first access, so a lost response is uncertain. The copy says only what is true: "SecurePay couldn't finish opening this invitation. You have not joined or confirmed anything. Try again." It never claims nothing changed.
- **401/403 are stage-aware** (they no longer share pre-Join wording):
  - Join 401 → sign-in, resume at the explicit Join button; "Nothing has been joined"; key released.
  - Join 403 (wrong account) → sign-in (the real session boundary; a successful OTP overwrites the session), resume at the Join button, invitation preserved; "This invitation isn't linked to the account you're signed in with. Sign in with the account it was sent to." Signing in never joins; the key is released. Previously the only action was "Try again" on the same wrong session.
  - Join succeeded, then the version read 401 → `state.join` kept, sign-in, `resume: 'reload-version'`: only the READ repeats, Join is never called again; wording "You have already joined this Agreement".
  - Post-Join read 403 → "The signed-in account can't open this Agreement right now." (no "nothing joined", nothing about the intended recipient).
  - Confirm 401 → Join and the reviewed version kept, sign-in, back to the exact review, another explicit press required; "Nothing was confirmed".
  - Confirm 403 → "The signed-in account can't confirm this Agreement. Nothing was confirmed." (no invitation-ownership wording).
...
- **Real API:** not run.
- **Scripted mock in real Chrome** (mock shaped from the controllers above; proxied via a scratch config, never port 8080): A signed-out → orientation → sign-in/OTP → Join → review → confirm (desktop); B already-signed-in skips auth (desktop); C version superseded → confirm sent once, current v2 shown with the changed notice, not auto-confirmed (desktop); D lost Join response → uncertain → retry with the same key (log: two joins, one key); E lost confirm response → not shown as confirmed → retry with same body → confirmed (log: same key/version/hash); F expired and not-found invitations fail closed at 320. Long-title/participant flow at **320 and 375**: no horizontal overflow. Revoked: covered by unit tests only (the browser read raced the page load). **Correction pass (scripted mock):** signed in as the wrong identity → Join 403 → sign-in screen with the wrong-account explanation → sign in as the intended identity → Join prompt (log: exactly one Join, the rejected one, before the explicit press) → explicit Join succeeds; and Join succeeds → version read 401 → sign-in with "already joined" wording → version reloaded (log: `invitation, join, version401, version` — one Join).
- **Render/unit:** `tests/ui-phase4.test.mjs` (33) + updated `tests/recipient.test.mjs`.
...
## K. Tests
602 tests, 0 failing (`node --test tests/*.test.mjs`); `tsc`, `eslint src`, `vite build` clean.
