...
- Automated (head after the correction pass): `node --test tests/*.test.mjs` 796 pass / 0 fail; `npx tsc -p tsconfig.app.json --noEmit` clean; `npx eslint src --quiet` clean; `npx vite build` ok. (`tsc --noEmit -p .` is **not** used as proof.) Older Money tests were updated to the withheld-command / caller-key contract.
...
Note (pre-existing, unchanged): the settlement-destination *read* gateway methods default to `currency = 'KES'`; only the register/replace form was made explicit.
